...
func TestUpdater_BulkUpdateTaskGroups_CreateInTheEnd(t *testing.T) {
	...
	gomock.InOrder(
		...
		s.EXPECT().CreateTaskGroup(ctx, &req.Create[0]).Return(&createdTaskGroup, nil),
		s.EXPECT().GetTaskGroups(ctx, &storage.GetTaskGroupsRequest{QuestID: questID}).Return([]*storage.TaskGroup{
			taskGroupsForTest[0],
			taskGroupsForTest[1],
			taskGroupsForTest[2],
...
func TestUpdater_BulkUpdateTaskGroups_UpdateWithoutReorder(t *testing.T) {
	...
	gomock.InOrder(
		...
		s.EXPECT().UpdateTaskGroup(ctx, &req.Update[0]).Return(&changedTaskGroup, nil),
		s.EXPECT().GetTaskGroups(ctx, &storage.GetTaskGroupsRequest{QuestID: questID}).Return([]*storage.TaskGroup{
			&changedTaskGroup,
			taskGroupsForTest[1],
			taskGroupsForTest[2],
...
func TestUpdater_BulkUpdateTaskGroups_DeleteReorderAndCreate(t *testing.T) {
	...
	gomock.InOrder(
		...
		s.EXPECT().UpdateTaskGroup(ctx, &req.Update[0]).Return(&updatedTaskGroups[0], nil),
		s.EXPECT().UpdateTaskGroup(ctx, &req.Update[1]).Return(&updatedTaskGroups[1], nil),
		s.EXPECT().UpdateTaskGroup(ctx, &req.Update[2]).Return(&updatedTaskGroups[2], nil),
		s.EXPECT().UpdateTaskGroup(ctx, &req.Update[3]).Return(&updatedTaskGroups[3], nil),
