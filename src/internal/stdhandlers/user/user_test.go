...
func TestUpdateHandler_HandleUser(t *testing.T) {
	...
	router.H().Use(jwt.StdMiddlewareStrict(jwtParser)).POST("/user/{id}", transport.WrapCtxErr(handler.HandleUser))
...
func TestUpdateHandler_HandlePassword(t *testing.T) {
	...
	router.Use(middleware.CtxLog(zaptest.NewLogger(t)))
	handler := NewUpdateHandler(factory, http.Client{}, pwHasher, jwtParser)
	router.H().Use(jwt.StdMiddlewareStrict(jwtParser)).POST("/user/{id}/password", transport.WrapCtxErr(handler.HandlePassword))
